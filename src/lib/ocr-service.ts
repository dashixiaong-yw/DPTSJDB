...
export class OCRService {
  ...
  constructor() {
    ...
    // 初始化模型列表（主模型 + 备用模型）
    // 可用视觉模型：Qwen3-VL-32B/8B/30B-A3B, PaddleOCR-VL-1.5(免费), DeepSeek-OCR, GLM-4.5V
    // 已下架模型（403）：Qwen2.5-VL-72B-Instruct, Pro/Qwen2.5-VL-7B-Instruct
    ...
    console.log(`[OCR服务] 模型配置来源: KIMI_VISION_MODEL=${process.env.KIMI_VISION_MODEL || '(未设置，使用代码默认值)'}, BACKUP_VISION_MODELS=${process.env.BACKUP_VISION_MODELS || '(未设置，使用代码默认值)'}`);
  }
  ...
  private markModelFailed(model: string): void {
    ...
    console.warn(`[OCR服务] 模型 ${model} 已标记为失败，5分钟内不再尝试`);
  }
  ...
  private isModelAvailable(model: string): boolean {
    ...
    // 失败超过5分钟后重新尝试（缩短缓存时间，避免一次失败导致后续全部不可用）
...
`;
  ...
  private isCacheComplete(cached: OCRResult | null, platform: string, imageType?: string): boolean {
    ...
    // 抖音平台：店铺月度数据截图和支出总额截图只需要 amounts
    if (platform === '抖音' && (imageType === '店铺月度数据截图' || imageType === '支出总额截图')) {
...
4. **金额字段**：
   ...
   - 金额必须转换为纯数字格式（去掉逗号、货币符号等格式符号。⚠️注意：逗号是千位分隔符（如3,628.75），不是小数点！去掉逗号后为3628.75，不可转换为362875）
   - 保留小数点后两位精度
...
截图中的金额通常包含千位分隔符，必须正确识别：

**正确示例：**
- 页面显示 "¥3,628.75" → 逗号是千位分隔符，点是小数点 → 输出 **3628.75**
- 页面显示 "¥87,819.34" → 逗号是千位分隔符，点是小数点 → 输出 **87819.34**
- 页面显示 "¥1,254.75" → 逗号是千位分隔符，点是小数点 → 输出 **1254.75**

**错误示例（必须避免）：**
- ❌ 把 "3,628.75" 识别成 "362875"（误将逗号和小数点都去掉）
- ❌ 把 "3,628.75" 识别成 "3628"（丢失小数部分）

**识别规则：**
1. 逗号 \`,\` 是**千位分隔符**，只起分组作用，提取时必须去掉
2. 点号 \`.\` 是**小数点**，必须保留，后面跟的是小数位
3. 最终输出必须是带小数的数字格式，如 3628.75

## 重要说明：
- 支出金额如果无法识别，必须设为0
- 金额必须转换为纯数字（去掉逗号、¥符号、负号等，但**保留小数点**）
- 只输出JSON，不要有任何其他文字`;
  }
...
2. **营业额**：
   ...
   - 金额必须转换为纯数字格式（去掉逗号、货币符号。⚠️注意：逗号是千位分隔符（如3,628.75），不是小数点！去掉逗号后为3628.75，不可转换为362875）
   - 保留小数点后两位
...
- 如果某项信息无法识别，对应字段设为null`;
   ...
   * 增加智能校验：修复千位分隔符误识别问题（如 3,628.75 被识别成 362875）
   */
  private normalizeAmounts(amounts: Record<string, unknown> | undefined): Record<string, number> | undefined {
    ...
    for (const [key, value] of Object.entries(amounts)) {
      if (typeof value === 'number' && !isNaN(value)) {
        normalized[key] = this.fixAmountError(key, value);
      } else if (typeof value === 'string') {
        // 去除逗号、空格、货币符号后解析
        const cleaned = value.replace(/[,，\s¥$￥]/g, '');
        const numValue = parseFloat(cleaned);
        if (!isNaN(numValue)) {
          normalized[key] = this.fixAmountError(key, numValue);
        } else {
  ...
  /**
   * 修复金额识别错误
   * 问题：LLM可能将 "3,628.75" 错误识别为 362875（丢失小数点）
   * 规则：如果金额 > 100000 且为整数（无小数位），可能是小数点丢失，尝试修复
   */
  private fixAmountError(key: string, value: number): number {
    // 只处理大于10万且看起来是整数的金额（可能是小数点丢失）
    if (value > 100000 && Number.isInteger(value)) {
      // 尝试除以100（如 362875 -> 3628.75）
      const fixed = value / 100;
      console.warn(`[OCR] 金额字段 "${key}" 的值 ${value} 疑似小数点丢失，已修正为 ${fixed.toFixed(2)}`);
      return fixed;
    }
    return value;
  }
